...
def main():
    ...
    ## 1. 교시 포인트 설정 ######################
    p1 = Position( 0, -300, 400.00, 0, 0, 0 )
    p2 = Position( 30, -200, 300, 0, 0, 0 )
    ## 2. 동작 조건 설정 ######################## 
    ...
    while True:
        ...
        dist_x, dist_y = map(float, response.split(','))
        print(f'dist_x: {dist_x}, dist_y: {dist_y}')

        p3 = Position( dist_x, dist_y, 400, 0, 0, 0 )

        print('p1 move start')
        rb.move(p1)
        print('p1 move finish')

        print('p2 move start')
        rb.move(p2)
        print('p2 move finish')
